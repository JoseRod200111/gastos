  ...
  const cargarDatos = async () => {
    let q = supabase
      ...
      .select(`
        ...
        empresa_id, division_id, categoria_id,
        proveedor_id, proveedores(nombre,nit),
    ...
    if (filtros.proveedor_nombre)
      q = q.ilike('proveedores.nombre', `%${filtros.proveedor_nombre.trim()}%`)
    if (filtros.proveedor_nit)
      q = q.ilike('proveedores.nit', `%${filtros.proveedor_nit.trim()}%`)
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .box  { border:1px solid #000; padding:14px; margin:10px; font-size:13px }
        ...
        <body>
          ...
          ${erogaciones.map(e=>`
            <div class="box">
              <div><span class="hdr">ID:</span> ${e.id} &nbsp;
                   <span class="hdr">Fecha:</span> ${e.fecha}</div>
              <div><span class="hdr">Empresa:</span> ${e.empresas?.nombre || '-'} &nbsp;
                   <span class="hdr">División:</span> ${e.divisiones?.nombre || '-'}</div>
              <div><span class="hdr">Categoría:</span> ${e.categorias?.nombre || '-'}</div>
              <div><span class="hdr">Proveedor:</span> ${e.proveedores?.nombre || '-'} &nbsp;
                   <span class="hdr">NIT:</span> ${e.proveedores?.nit || '-'}</div>
              <div><span class="hdr">Total:</span> Q${e.cantidad?.toFixed(2)}</div>
              <div><span class="hdr">Observaciones:</span> ${e.observaciones || 'N/A'}</div>

              <table>
                <thead>
                  <tr><th>Concepto</th><th>Cant.</th><th>P.Unit</th>
                      <th>Importe</th><th>Pago</th><th>Doc.</th></tr>
                </thead>
                <tbody>
                  ${(detalles[e.id]||[]).map(d=>`
                    <tr>
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <button onClick={descargarPDF} className="ml-2 bg-green-600 text-white px-4 py-2 rounded">📄 Generar PDF</button>
        <button onClick={()=>window.location.href='/dashboard'} className="ml-2 bg-gray-700 text-white px-4 py-2 rounded">⬅ Volver</button>
      </div>
          ...
          erogaciones.map(e=>(
            <div key={e.id} className="box border p-4 my-4 text-sm">
              <div className="grid grid-cols-2 gap-2 mb-2">
                ...
                <div className="col-span-2">
                  <span className="font-semibold">Observaciones:</span> {e.observaciones}</div>
              </div>

              <table className="w-full border text-sm">
                <thead>
                  <tr className="bg-gray-200">
                    <th>Concepto</th><th>Cant.</th><th>P.Unit</th>
                    <th>Importe</th><th>Pago</th><th>Doc.</th>
                  </tr>
                </thead>
                <tbody>
