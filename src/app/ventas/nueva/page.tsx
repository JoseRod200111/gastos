...
export default function NuevaErogacion() {
  ...
  const [form, setForm] = useState({
    ...
    proveedor_id: '', fecha: '', cantidad: 0, observaciones: ''
  })
  ...
  const [nuevoProv, setNuevoProv] = useState({ nombre:'', nit:'', direccion:'', contacto_nombre:'', telefono:'' })
  ...
  /* recalcula total */
  useEffect(() => {
    const total = detalles.reduce((sum, d) => sum + Number(d.cantidad || 0) * Number(d.precio_unitario || 0), 0)
  ...
  const guardarErogacion = async () => {
    try {
      ...
      // 2) detalles: NO enviar "importe" (columna generada en la BD)
      const payload = detalles.map(d => ({
        erogacion_id : (erog as any).id,
        producto_id  : d.producto_id ? Number(d.producto_id) : null, // null si no hay producto
        concepto     : d.concepto,
      ...
      const { error: detErr } = await supabase.from('detalle_compra').insert(payload)
      if (detErr) throw new Error(`detalle: ${detErr.message}`)

      // El trigger AFTER INSERT en detalle_compra se encargará de crear entradas en inventario_movimientos
      alert('Erogación guardada correctamente')
      router.push('/menu')
    } catch (e: any) {
      alert(`Error al guardar detalle: ${e.message ?? e}`)
    }
  }
