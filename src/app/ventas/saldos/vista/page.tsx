...
async function fetchAllDetalleVentaCliente(clienteId: number) {
  const all: any[] = []
  let from = 0
...
function VistaDeudasClienteInner() {
  ...
  const cargarVentasCredito = useCallback(async (id: number) => {
    ...
    try {
      ...
      const { data: rowsDV, error: dvErr } = await fetchAllDetalleVentaCliente(id)
      ...
      for (const r of (rowsDV || []) as any[]) {
        ...
        const formaPagoRel = Array.isArray(r.forma_pago) ? r.forma_pago[0] : r.forma_pago
        const metodo = String(formaPagoRel?.metodo || '').toLowerCase()
        const imp = toNum(r.importe)

        const esPendiente =
          (fpId != null && idsPendientes.has(fpId)) ||
      ...
      if (vIds.length > 0) {
        const { data: pagosRows, error: pErr } = await fetchAllPagosVenta(vIds)
