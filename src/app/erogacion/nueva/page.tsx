...
export default function NuevaErogacion() {
  ...
  /* ───────────── detalles (artículos) ─────────────
     Si producto_id tiene valor, esa línea afectará inventario (entrada).
     Si va vacío, se mantiene como concepto libre (sin inventario).
  */
  ...
  const handleDetalleChange = (i: number, field: string, val: any) => {
    setDetalles((prev) => {
      ...
      // si eligen producto y el concepto está vacío, lo rellenamos con el nombre del producto
  ...
  const guardarNuevoProveedor = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        contacto_nombre: nuevoProv.contacto_nombre,
  ...
  const guardarErogacion = async () => {
    ...
    // 1) cabecera
    const { data: erog, error } = await supabase.from('erogaciones').insert([form]).select().single()
    if (error) {
      alert('Error al guardar la erogación')
      return
    }

    // 2) detalles (con producto_id opcional). Enviamos importe calculado.
    const det = detalles.map((d) => ({
      ...d,
      erogacion_id: (erog as any).id,
      producto_id: d.producto_id ? Number(d.producto_id) : null,
      importe: Number(d.cantidad) * Number(d.precio_unitario),
    }))
    ...
    /* IMPORTANTE
       Si existe el trigger AFTER INSERT en detalle_compra (como lo definimos),
       las líneas con producto_id NO nulo generan automáticamente:
       INSERT INTO inventario_movimientos(producto_id,tipo,cantidad,erogacion_detalle_id)
       VALUES (NEW.producto_id, 'ENTRADA', NEW.cantidad, NEW.id);
    */

    alert('Erogación guardada correctamente')
    router.push('/menu') // volvemos al menú nuevo
  }
        ...
        {showNuevoProv && (
          <div className="border p-3 rounded bg-gray-50 space-y-2">
            ...
            <input
              ...
              onChange={(e) => setNuevoProv({ ...nuevoProv, contacto_nombre: e.target.value })}
            />
            <input
              className="border p-2 w-full"
            ...
            />
            <button onClick={guardarNuevoProveedor} className="w-full bg-blue-600 text-white py-2 rounded">
              Guardar Proveedor
            </button>
          </div>
        )}
